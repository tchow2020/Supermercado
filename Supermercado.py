...
while True:
    if options == 'i':
      ...
      options = input('Selecione uma opção \n[i]nserir [a]pagar [l]istar [s]air: ').lower()

    elif options == 'a':
      remove_item = input('Digite qual item você vai apagar: ')

      # Remover pelo número/posição
      try:
        position = int(remove_item)
        del buy_list[position]
        print(f'item {buy_list[position]} foi removido com sucesso!')
      except ValueError:
